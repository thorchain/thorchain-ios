...
public struct BaseAmount : ExpressibleByIntegerLiteral, ExpressibleByStringLiteral {
    ...
    public var assetAmount : AssetAmount {
        ...
        if let quotientDecimal : Decimal = Decimal(string: quotientString),
           let remainderDecimal : Decimal = Decimal(string: remainderString),
           let zerosDecimal : Decimal = Decimal(string: zerosString) {
            ...
            let fraction : Double = Double(quotient) + Double(remainder) / Double(zeros)
    ...
    public init(integerLiteral: Int) {
        self.amount = BigInt(integerLiteral)
        self.decimals = AssetAmount.defaultDecimal
    }
    
    /// Ability to represent BaseAmount as a String, e.g. "100000000000"
    public init(stringLiteral: String) {
        self.amount = BigInt(stringLiteral: stringLiteral)
        self.decimals = AssetAmount.defaultDecimal
    }
}
...
public struct AssetAmount {
    ...
    public var baseAmount : BaseAmount {
        ...
        let bigIntFromString : BigInt = BigInt(stringLiteral: baseAmountRoundedString)
